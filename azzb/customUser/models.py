...
class FollowThrough(models.Model):
    ...
    followee = models.ForeignKey(
        "Profile", on_delete=models.CASCADE, related_name="followee_set"
    )
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["follower", "followee"], name="follow_once"
            ),
            models.CheckConstraint(
                check=~Q(follower=F("followee")), name="not_follow_self"
            ),
        ]
...
class Profile(models.Model):
    ...
    username = models.CharField(
        "Отображаемое имя пользователя", max_length=150, default=None, blank=True
    )
    avatar = models.ImageField(
        default="default/default_user_avatar.jpg", upload_to=user_profile_avatar_path
    )
